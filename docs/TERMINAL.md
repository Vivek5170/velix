...
Tool approval is now handled via a generalized interactive ask_user flow. When an agent attempts
an action that requires user authorization, the system sends an ASK_USER_REQUEST message to the
gateway which prompts the user. The user's reply is delivered back to the requesting process as
an ASK_USER_REPLY message (both messages include a trace id to correlate request and reply).

Gateway commands `/approve` and `/deny` are provided by the terminal client for convenience
but the canonical protocol is ASK_USER_REQUEST / ASK_USER_REPLY over the internal bus.
...
Behavior:
- The requesting process must register an on_bus_event hook (or rely on trace-based routing)
  and wait for the ASK_USER_REPLY with matching `payload.trace` to continue.
- The executioner injects environment variables for runtime configuration (see Setup notes).
