...
std::string request_llm_summary(const nlohmann::json& older_turns,
	...
	if (!previous_summary.empty()) {
		prompt
			<< "Update the following technical handoff summary by incorporating the new conversation turns.\n\n"
			<< "PREVIOUS SUMMARY:\n" << previous_summary << "\n\n"
		...
		prompt
			<< "Create a structured technical handoff summary for a later assistant that will continue this conversation after earlier turns are compacted.\n\n"
			<< "TURNS TO SUMMARIZE:\n" << content_to_summarize << "\n\n"
